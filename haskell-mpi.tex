...
\section{The trapezoid method for computing definite integrals}

We now consider the problem of computing definite integrals
using the trapezoid method. The algorithm is naturally data parallel, and is a common example
used to illustrate parallel programming, for example, see Pacheo's textbook on MPI~\cite{Pacheo}.
We can approximate $\int_{x=a}^{b} f(x)$
by summing the area of a consecutive sequence of regularly spaced trapezoids lying under $f$,
starting at $a$ and ending at $b$ (Figure \ref{trapezoidfig}). A single trapezoid spanning the interval
$[x_0,x_1]$ has area $h/2(f(x_0) + f(x_1))$. Extending this to $n$ subintervals $[x_0,x_1,\ldots,x_n]$
