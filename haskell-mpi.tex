...
MPI uses a more complex two-level numbering scheme.
...
called known as the \emph{world}.

\Todo{Can we clarify this?}
MPI allows synchronous and asynchronous messaging, but even in
synchronous mode there is no requirement for
...
Besides simple process-to-process messaging, MPI
...
It might seem strange at first that processes do not exchange any
...
\emph{Single Program Multiple Data} (SPMD) style. MPI also allows
the \emph{Multiple Program Multiple Data} (MPMD) style, where
...
only the master process calls |recv| and prints the result, whilst
all processes except for the master call |send|.
While programming with MPI in C, care must
be taken to avoid allocations/computations in processes that would not
actually use the appropriate data, and to avoid using
