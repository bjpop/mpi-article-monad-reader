...
\begin{enumerate}
   \item GHC 7.0.3, with optimisation flags |-O2|.
   \item GCC 4.4.4, with optimisation flags |-O2|.
   \item OpenMPI 1.4.2.
\end{enumerate}
...
when integtating Sine on the interval $[0,1000 \pi]$ using $10^9$ trapezoids. \label{TiminigTable}}
\end{figure}
...
simulataneous runs of the same computation. Figure~\ref{PerformanceGraph}
...
of the threaded version to a single shared-memory instance will utlimately be
...
performance costs due to the need to make a copy of the data for serialisation.
Furthermore, in most cases, each message sent is preceeded implicitly by another
message carrying size information about the serialised data stream.
...
the need for serialization. ByteStrings, unboxed arrays, and members of typeclass
...
their favourite language today. The current version of the library covers the most commonly used parts of
