...
\end{equation*}
Extending this to $n$ equally spaced subintervals $[x_0,x_1,\ldots,x_n]$ we arrive at the formula:
\begin{equation*}
\begin{split}
...
\end{split}
\end{equation*}
Listing \ref{SequentialTrap} provides a sequential implementation of the trapezoid method in Haskell, with
...
f x = 4 / (1 + x * x)
\end{Verbatim}
\caption{Sequential program for calculating definite integrals using the trapzoid method. \label{SequentialTrap}}
\end{listing}
...
Given that the computations on each sub-interval are independent, the trapezoid method is a classic
data-parallel problem, which can be parallelized using a simple chunking scheme as follows:
\begin{enumerate}
   \item The master processor splits the problem into $n/p$ evenly sized chunks, and distributes them across all processors.
   \item In parallel, each processor $i$ computes the result for chunk $i$.
   \item The master processor collects the results computed for each chunk and computes the final answer.
\end{enumerate}
...
\end{Verbatim}
\caption{Multi-threaded parallel program for calculating definite integrals using the trapzoid method. \label{SequentialTrap}}
\end{listing}
